...
# from motor.motor_asyncio import AsyncIOMotorClient  # MongoDB - commented out
import os
import logging
...
# MongoDB connection - commented out (not using database)
...
def natural_sort_key(table):
    """Natural sort key for table numbers like T-1, T-2, T-10, B-1, etc."""
    table_no = table.get("table_no", "")
    # Split into text and number parts
    parts = re.split(r'(\d+)', table_no)
...
async def get_tables(authorization: Optional[str] = Header(None)):
    ...
    # Sort tables naturally (T-1, T-2, ... T-9, T-10, T-11...)
    tables.sort(key=natural_sort_key)
...
async def create_order(order_input: OrderCreate, authorization: Optional[str] = Header(None)):
    ...
    if pos_result.get("success"):
        ...
        # MongoDB storage commented out - not using database
...
# MongoDB shutdown - commented out (not using database)
