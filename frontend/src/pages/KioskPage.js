...
const KioskPage = () => {
  ...
  // Compact Menu Item Card for Portrait (4 per row)
  const PortraitMenuCard = ({ item }) => {
    ...
    return (
      ...
      >
        ...
        <div className={`h-[70px] overflow-hidden ${inCart ? 'opacity-80' : ''}`}>
          <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
        </div>
        <div className="p-1.5 text-center">
          <h3 className="font-heading font-semibold text-[11px] truncate uppercase text-blue-dark">{item.name}</h3>
          <button
            onClick={(e) => { e.stopPropagation(); setSelectedItem(item); }}
            className={`mt-1 w-7 h-7 mx-auto rounded-full flex items-center justify-center transition-all ${
              inCart ? 'bg-blue-light/30 text-blue-medium' : 'bg-blue-hero text-white'
            }`}
          >
            <Plus size={14} />
          </button>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${
                inCart ? 'bg-blue-light/30 text-blue-medium hover:bg-blue-hero hover:text-white' : 'bg-blue-hero text-white hover:bg-blue-medium'
              }`}
            >
              <Plus size={16} />
            </button>
          ...
          >
            <div className="p-4 portrait:p-4 landscape:p-6 border-b border-border flex items-center justify-between bg-white">
              <div>
                ...
                <p className="text-muted-foreground mt-1 text-sm">Tap on your table number</p>
              </div>
              <button onClick={() => { touchSound.playClick(); setShowTableSelector(false); }} className="p-3 hover:bg-muted rounded-sm">
                ...
                {(() => {
                  ...
                  return Object.keys(sections).sort((a, b) => a === '' ? 1 : b === '' ? -1 : a.localeCompare(b)).map(section => (
                    <div key={section || 'no-section'} className="mb-6">
                      {section && <h3 className="text-base font-semibold mb-3 text-blue-dark uppercase">{section}</h3>}
                      <div className={`grid gap-2 ${isPortrait ? 'grid-cols-5' : 'grid-cols-8 md:grid-cols-10'}`}>
                        {sections[section].map((table) => (
                          <motion.button
                            ...
                            className={`aspect-square rounded-lg text-base font-bold transition-all shadow-sm flex items-center justify-center ${
                              tableNumber === table.table_no
                                ? 'bg-blue-hero text-white shadow-lg ring-4 ring-blue-hero/30'
                                : 'bg-white hover:bg-blue-light/20 border border-border'
                            }`}
                          >
                            {table.table_no}
                          </motion.button>
            ...
            {tableNumber && (
              <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="p-4 portrait:p-4 landscape:p-6 border-t border-border bg-white">
                <button
                  onClick={() => { touchSound.playClick(); setShowTableSelector(false); setTimeout(() => handlePlaceOrder(), 100); }}
                  disabled={isPlacingOrder}
                  className={`w-full max-w-md mx-auto block bg-blue-hero text-white py-4 rounded-sm font-semibold hover:bg-blue-medium ${isPortrait ? 'text-base' : 'text-xl'}`}
                >
